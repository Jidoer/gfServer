...
func (c *Control) GetServerList() *[]*protorpc.Server {
	var mList []*protorpc.Server
	list_ := c.GetServerListForNeb()
	for _, s_ := range *list_ {
		sub_list := protorpc.Server{
			Ip:       s_.IP,
			Name:     s_.Name,
			Port:     int32(s_.Port),
...
func (c *Control) GetClientList() *[]*client.Client {
	var mList []*client.Client
	c.ClientLock.RLock()
	for _, sub_c := range c.Clients {
		mList = append(mList, sub_c)
	}
	c.ClientLock.RUnlock()
	return &mList
}

func (c *Control) GetSubServerClientList() *[]*client.Client {
	var mList []*client.Client
	c.ClientLock.RLock()
	for _, sub_c := range c.Clients {
		if sub_c.Auth == 2 && sub_c.Service != nil {
